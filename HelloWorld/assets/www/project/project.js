if (window.appContext.getObject("projectjs") == undefined) {
	...
	var projectjs = {
		...
		listProject: function(pageNo) {
			...
			$.getJSON(serverAddress + "/project/list.do?pageNo=" + pageNo, function(result) {
		...
		countProject: function() {
			var that = this;
			$.getJSON(serverAddress + "/project/count.do", function(result) {
				if(result.status == "success") {
		...
		updateProject: function() {
			var that = this;
			$.post(serverAddress + "/project/update.do", 
					{
						no: $("#no").val(),
						title: $("#title").val(),
		...
		addProject: function() {
			var that = this;
			$.post(serverAddress + "/project/add.do", 
					{
						title: $("#title").val(),
						content: $("#pcontent").val(),
						startDate: $("#startDate").val(),
		...
		viewDetailProject: function (projectNo) {
			$.getJSON(serverAddress + "/project/view.do?no=" + projectNo, function(result) {
				if(result.status == "success") {
		...
		deleteProject: function() {
			var that = this;
			$.getJSON(serverAddress + "/project/delete.do?no=" + $("#no").val(), function(result) {
				if(result.status == "success") {
					that.listProject();
					$("#view").trigger("projectChanged");
				} else {
